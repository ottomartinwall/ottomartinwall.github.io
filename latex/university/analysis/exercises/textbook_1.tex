\chapter{My Answers to Exercises From a Few Textbooks}
...
\begin{proof}(\textbf{3})
	Proof 2 have already shown that $f(A)\backslash f(C)\subseteq f(A\backslash C)$, so to prove that $f(A)\backslash f(C) = f(A\backslash C)$ I must only prove the converse of the first statement.
